...
FMOD_SYSTEM* SoundManager::m_pSystem = nullptr;
FMOD_CHANNEL* SoundManager::m_pChannel[] = {};
std::map<std::string, FMOD_SOUND*> SoundManager::m_sounds = {};
...
bool SoundManager::Initialize(const std::string& _folderPath)
{
...
void SoundManager::AddSound(std::string _soundPath, bool _bLoop)
{
...
void SoundManager::Play(CHANNEL _ch, const std::string& _soundPath)
{
	FMOD_SOUND* sound = nullptr;
...
void SoundManager::Pause(CHANNEL _ch)
{
	FMOD_Channel_SetPaused(m_pChannel[_ch], true);
}

void SoundManager::Resume(CHANNEL _ch)
{
	FMOD_Channel_SetPaused(m_pChannel[_ch], false);
}

void SoundManager::Stop(CHANNEL _ch)
{
	FMOD_Channel_Stop(m_pChannel[_ch]);
}

void SoundManager::SetVolume(CHANNEL _ch, float _vol)
{
